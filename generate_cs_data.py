...
def get_satisfaction(status, inquiry_type):
    ...
    if inquiry_type == "서비스불만":
        base -= 0.5
    score = int(np.clip(np.round(np.random.normal(base, 0.8)), 1, 5))
    return score
...
for i in range(1000):
    ...
    receipt_dt = start_date + timedelta(minutes=minutes_offset)
    ...
    satisfaction = get_satisfaction(status, inq_type) if status == "처리완료" else None
...
df.to_csv("cs_inquiries.csv", index=False, encoding="utf-8-sig")
print(f"cs_inquiries.csv 생성 완료: {len(df)}행")
